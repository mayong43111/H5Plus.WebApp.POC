document.addEventListener('plusready', function () {   //固定模板,不用修改
    ...
    var _BARCODE = 'XueDaFeature' // ① 插件名称(很重要),变量名和字符串值可自定义
    var bridge = window.plus.bridge //②固定模板,不用修改,
    var XueDaFeature = { //③变量名可自定义
        ...
        //人脸登录验证
        startLoginDetectFace: function (callback) {
            this.callNative('startLoginDetectFace', [], callback)
        }
    };
    window.plus.XueDaFeature = XueDaFeature //固定模板,示例:window.plus.插件名 = 同注释③变量名一致
}, true);